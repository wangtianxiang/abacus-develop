...
struct veff_pw_batch_op {
    ...
    /// \param in : input array, elecstate::Potential::v_effective
    /// \param batchSize : batch size for once calcution
    ///
    /// Output Parameters
    /// \param out : output array
    void operator() (
    ...
    /// \param size : array size
    /// \param ld_out : leading dimension of out
    /// \param ld_out1 : leading dimension of out1
    /// \param in : input array, elecstate::Potential::v_effective
    /// \param batchSize : batch size for once calcution
    ///
    /// Output Parameters
...
template <typename FPTYPE>
struct veff_pw_batch_op<FPTYPE, base_device::DEVICE_GPU>
{
    void operator()(const base_device::DEVICE_GPU* dev, const int& size, std::complex<FPTYPE>* out, int ld_out, const FPTYPE* in, int batchSize);
