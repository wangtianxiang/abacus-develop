                                                                    ...
                                                                    const int batchSize)
{
    ...
    hipLaunchKernelGGL(HIP_KERNEL_NAME(set_3d_fft_box_batch<FPTYPE>), block, dim3(THREADS_PER_BLOCK), 0, 0,
        npwk,
        box_index,
                                                                              ...
                                                                              const int batchSize)
{
    ...
    hipLaunchKernelGGL(HIP_KERNEL_NAME(set_recip_to_real_output_batch<FPTYPE>), block, dim3(THREADS_PER_BLOCK), 0, 0,
        nrxx,
        add,
        factor,
        reinterpret_cast<const thrust::complex<FPTYPE>*>(in), ld_in,
...
{
    dim3 block((npwk + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK, 1, batchSize);
    hipLaunchKernelGGL(HIP_KERNEL_NAME(set_real_to_recip_output_batch<FPTYPE>), block, dim3(THREADS_PER_BLOCK), 0, 0,
        npwk,
        nxyz,
        add,
        factor,
